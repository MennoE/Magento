...
/**
 * overrided to add dispatch an event so we dont' have to override the grid class
 * every time we have to add a new column
 *
 * this method of overriding was used instead of using config.xml <rewrite>
 * because when a class extends Mage_Adminhtml_Block_Widget_Grid
 * it will use Mage_Adminhtml_Block_Widget_Grid even if
 * the Mage_Adminhtml_Block_Widget_Grid is overrided by another class
 */
    ...
    */
    ...
    protected function _prepareCollection()
    {
        ...
        if ($this->getCollection()) {
            ...
            Mage::dispatchEvent('adminhtml_grid_prepare_collection_before', array('block'=>$this->getCollection()));

            if ($this->getId()) {
                Mage::dispatchEvent('adminhtml_grid_prepare_collection_before_'.$this->getId(),
                                    array('block'=>$this));
            }
    ...
    {
        Mage::dispatchEvent('adminhtml_grid_prepare_columns_before', array('block'=>$this));

        if ($this->getId()) {
            Mage::dispatchEvent('adminhtml_grid_prepare_columns_before_'.$this->getId(),
                                array('block'=>$this));
        }
    ...
    protected function _prepareMassactionBlock()
    {
    	...
    	if ($this->getId()) {
    		// Added dispatchEvent, to make it simple to add extra mass actions to the grid page.
			Mage::dispatchEvent('adminhtml_grid_prepare_massaction_block_after_' . $this->getId(),
				array('block'=>$this));
		}
