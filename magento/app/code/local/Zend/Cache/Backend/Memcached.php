...
/**
 * @extended due to numerous ZF bugs
 * Magento uses a very old version of ZF, which contains quite some bugs i.e.
 * - Filling percentage calculation problem (throws exception)
 * - undefined index notice 'bytes' fix
 * - undefined index notice 'limit_maxbytes' fix
 * - Socket errors when loading or deleting cache entries
 */
...
class Zend_Cache_Backend_Memcached extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
	 ...
	 * @EXTENDED: added try catch to prevent socket errors from killing scripts
     *
	 ...
	 *
	 * @hack randomly the server runs into the problem that 'limit_maxbytes'
	 * is not defined. This results in 'undefined index' message on page.
	 * So when this happen we set this to the limit we also configured in memcache.
     */
    public function getFillingPercentage()
    {
        ...
        foreach ($mems as $key => $mem) {
			...
			// hack: read method description
			if (empty($mem['limit_maxbytes'])) {
				$mem['limit_maxbytes'] = 2147483648;
			}
